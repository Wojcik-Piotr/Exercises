package pl.sdacademy.exercises.homework.Zad20;

public class SudouVerifier {

    protected boolean sudokuVerifier(int[][] board) {
        return rowVerifier(board) && columnVerifier(board) && squareVerifier(board);
    }


    private boolean rowVerifier(int[][] board) {
    ...
    private boolean columnVerifier(int[][] board) {
        if (board == null || board.length != 9 || board[0].length != 9) {
            return false;
        } else {
            for (int row = 0; row < 9; row++) {
    ...
    private boolean squareVerifier(int[][] board) {
        if (board == null || board.length != 9 || board[0].length != 9) {
            return false;
        } else {
